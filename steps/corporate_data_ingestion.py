...
def process_collection(collection_name, ingesters, configuration, spark_session, hive_session):
    ...
    for export_date in export_date_range:
        ...
        ingester = ingesters.get(collection_name, BaseIngester)(configuration, spark_session, hive_session)
        logger.info(f"{ingester.__class__.__name__}: ingester initialised")
        logger.info(f"{ingester.__class__.__name__}: ingester running")
        ingester.run()
        logger.info(f"{ingester.__class__.__name__}: ingester completed")
