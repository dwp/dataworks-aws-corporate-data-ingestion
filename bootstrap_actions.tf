  ...
  content = templatefile("${path.module}/bootstrap_actions/download_scripts.sh",
    {
      ...
      calculation_parts_ddl_files = [
        for ddl_file in data.aws_s3_object.calculation_parts_ddl :
        format("s3://%s/%s", data.terraform_remote_state.common.outputs.config_bucket.id, ddl_file.key)
      ]
  })
  tags = {
