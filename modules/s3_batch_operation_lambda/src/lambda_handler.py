...
target_bucket = str(os.environ["DESTINATION_BUCKET_ARN"]).split(":")[-1]
...
transfer_config = TransferConfig(
    max_concurrency=my_max_concurrency, multipart_chunksize=my_multipart_chunksize
)
config = Config(
    max_pool_connections=my_max_pool_connections,
    ...
    try:
        ...
        if s3VersionId is not None:
            ...
            if metadata_copy == "Enable":
                get_metadata = s3Client.head_object(
                    Bucket=s3Bucket, Key=s3Key, VersionId=s3VersionId
                )
            # Construct/Retrieve get source key tagging
            if tagging_copy == "Enable":
                get_obj_tag = s3Client.get_object_tagging(
                    Bucket=s3Bucket, Key=s3Key, VersionId=s3VersionId
                )
        else:
            # Construct/Retrieve get source key metadata
        ...
        if tagging_copy == "Enable":
            ...
            tagging_to_s3 = "&".join(
                [
                    f"{parse.quote_plus(d['Key'])}={parse.quote_plus(d['Value'])}"
                    for d in existing_tag_set
                ]
            )
            # Construct Request With Required and Available Arguments
            if existing_tag_set:
        ...
        logger.error(
            f"Unable to complete requested operation, see Clienterror details below:"
        )
        try:
            ...
            errorS3ExtendedRequestID = e.response.get("ResponseMetadata", {}).get(
                "HostId"
            )
            resultCode = "PermanentFailure"
            resultString = "{}: {}: {}: {}".format(
                errorCode, errorMessage, errorS3RequestID, errorS3ExtendedRequestID
            )
        ...
        # log errors, some errors does not have a response, so handle them
        logger.error(
            f"Unable to complete requested operation, see Additional Client/Service error details below:"
        )
        try:
            logger.error(e.response)
            errorCode = e.response.get("Error", {}).get("Code")
        ...
        results.append(
            {"taskId": taskId, "resultCode": resultCode, "resultString": resultString}
        )
...
if __name__ == "__main__":
    logger.info("Lambda start_backup started")
