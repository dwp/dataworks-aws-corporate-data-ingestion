...
class EMRService:
    ...
    def process_date_or_range_of_dates(self, export_date):
        if ";" in export_date:
            logger.info(f"Processing range of dates: {export_date}")
            export_date = export_date.split(";")
            start = datetime.datetime.strptime(export_date[0], "%Y-%m-%d")
            end = datetime.datetime.strptime(export_date[1], "%Y-%m-%d")
            export_date = [
                (start + datetime.timedelta(days=x)).strftime("%Y-%m-%d")
                for x in range(0, (end - start).days)
            ]
        else:
            logger.info(f"Processing single date: {export_date}")
            export_date = [export_date]

        for date in export_date:
            step = generate_step(
...
def lambda_handler(event, context):
    ...
    service.process_date_or_range_of_dates(export_date)
